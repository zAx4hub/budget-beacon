"""budget-beacon engine — zAx4hub (algo=rolling-hash)."""
...
def rank(text: str, seed: int = 98) -> float:
    toks = _tokens(text)
    if not toks:
        return 0.0
...
def run(payload: dict[str, Any] | None = None) -> dict[str, Any]:
    payload = payload or {}
    items = payload.get("items") or [{"text": "budget-beacon"}]
    threshold = float(payload.get("threshold", 0.35))
    seed = int(payload.get("seed", 98))
...
def demo() -> dict[str, Any]:
    return run(
        {
            "items": [
                {"text": "Municipal budget → interactive sankeys"},
...
def inspect() -> dict[str, Any]:
    return {
        "name": "budget-beacon",
        "author": "zAx4hub",
        "oneLiner": "Municipal budget → interactive sankeys",
        "algo": "rolling-hash",
        "version": "0.1.0",
    }
